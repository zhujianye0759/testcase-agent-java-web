...
- [x] 6.4 对读者可见叙述字段建立共享安全边界：新结果中的内部 stable key/诊断占位符在持久化前失败关闭，绑定字段保持可用；既有投影确定性读者化，中文斜杠枚举不得误报 Unix 路径。增加同 task 制品重生成端点，只允许已有制品的完成 structured ALL，按旧 artifact_id 做 CAS，成功/不存在/冲突分别返回 204/404/409，不调用 KEE、不改业务表。验证：`SensitiveValueRedactorTest,FeatureReconciliationValidatorTest,RequirementMaterialReviewValidatorTest,StructuredWorkbookExporterTest,GenerationTaskDetailTest,GenerationWorkflowAllModeTest,GenerationTaskRepositoryRetryTest,GenerationTaskControllerArtifactRegenerationTest`。 `[Req-ID]: REQ-STG-001, REQ-SGD-001, REQ-SGD-003, REQ-SGD-005`
...
- [x] 7.3 在 KEE 集成提交 `775ad6c6f4591cf45e2a8b23be0b81ce8107ec64`（包含 `4c68f2f8`、前置 `ba21fecf`）对应镜像获得精确证据后，执行 parsed-units 全遍历、三个 Skill（含双 operation）、十类错误、一次 repair 标志、同步响应、普通 Agent 非干扰，以及同任务安全重生成页面/Excel 制品的联合验收；记录提交/镜像和运行证据。部署或合格材料条件不足时保持本任务未完成。证据：`evidence/7.3-joint-e2e-preflight.md`。 `[Req-ID]: REQ-SKI-001~006, REQ-SGD-004, REQ-SGD-005`
