...
- 审查与生成阶段均先创建隔离的 Skill 准备会话：审查阶段显式传入 `feature-scope-reconciliation`，生成阶段显式传入 `functional-testcase-design`；仅在准备 SSE 中观察到对应名称的 `read_skill` 成功后，才在同一会话处理连续的有界业务请求。
