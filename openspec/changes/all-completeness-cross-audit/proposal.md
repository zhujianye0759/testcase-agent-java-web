...
- 最终双向核对以全量候选上下文和固定 16 个目标候选页面执行；每页经严格合同验证后以稳定锚点在 Java 端归并，所有页面成功前不持久化。最终详情、Markdown 和工作簿确定性清理 `documentId`、`unitId`、`candidateIds`、`groupAnchorId` 机器绑定，不得恢复已废弃的模型 JSON 协议，也不得新增技术工作表。
