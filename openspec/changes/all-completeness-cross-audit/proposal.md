...
- 每个有界审查单元、最终双向核对和单功能生成批次均先创建独立的 Skill 准备会话：审查显式传入 `feature-scope-reconciliation`，生成显式传入 `functional-testcase-design`；仅在准备 SSE 中观察到对应名称的 `read_skill` 成功后，才在同一会话处理该唯一业务请求并立即关闭，绝不跨业务项复用。任一准备失败均拒绝整个当前审查或生成阶段，不继续领取后续单元或批次。
...
- 保留现有固定 Markdown 生成响应的两张表和 Excel 的两个 Sheet；不得恢复已废弃的模型 JSON 协议，也不得新增技术工作表。
